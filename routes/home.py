...
from helpers import client, send_message, spreadsheet_ids_and_ranges, format_phone_number, format_move_date, send_email, send_to_gronat, send_to_sheets
...
def add_data():
    ...
    if request.method == 'POST':
        ...
        ########send to sheets
        # default value '0' if not sent to sheet, '1' if sent.
        sent_to_sheets = '0'
        lead_cost = domain_settings.lead_cost if domain_settings else "110"
        spreadsheet_config = spreadsheet_ids_and_ranges.get(label)
        sent_to_sheets_success = send_to_sheets(spreadsheet_config,timestamp,first_name,ozip,dzip,dcity,dstate,data,ref_no,validation,label, phone_number,lead_cost, send_to_google_sheet)
